...
KTX·SRT 스킬은 Python 3.11 이상과 [`uv`](https://docs.astral.sh/uv/getting-started/installation/)가 필요합니다. `uv`가 helper에 선언된 `korail2`·`SRTrain` 익명 search client를 자동 설치하며 철도 회원 ID·비밀번호는 필요하지 않습니다.
