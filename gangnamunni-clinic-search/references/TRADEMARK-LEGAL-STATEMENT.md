# 상표 사용 법적 고지 — `gangnamunni-clinic-search`

이 스킬에서 `강남언니` 명칭은 병원 후보·평점·리뷰·지원 언어 조회의 **대상 검색 서비스**를 식별하기 위해 사용한다. k-skill의 출처를 강남언니 운영사로 표시하려는 사용이 아니다.
...
따라서 이 명칭은 필요한 범위의 평문으로만 사용하고, 별도 근거 없이 로고, 공식·제휴·후원·인증·파트너 표현을 사용하지 않는다. 각 상표의 권리는 해당 권리자에게 있다. 이 문서는 의료 적합성, 저작권, 데이터베이스권, 부정경쟁방지법, 약관·계약, 개인정보 또는 접근 방식의 적법성을 판단하지 않는다.
