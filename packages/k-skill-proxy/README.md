...
- `KSKILL_PROXY_TRUST_PROXY_HOPS` — Fastify가 신뢰할 reverse-proxy hop 수, 기본 `0`. gpu01 Cloudflare Tunnel 배포는 **반드시 `1`**. 직접 노출되는 로컬 서버에서는 설정하지 않는다. hops가 1 이상이면 rate limiter는 `CF-Connecting-IP`를 `X-Forwarded-For`보다 우선한다.
