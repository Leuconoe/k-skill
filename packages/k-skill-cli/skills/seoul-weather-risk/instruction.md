...
ASK 서울의 장소별 기상 위험 예상 시간대 단일 제품(`weather_place_risk_window`)을 읽기 전용 스킬로 탐색한다. helper는 ASK Seoul의 인증된 HTTPS API만 호출하며, 실패 또는 미준비 상태를 fixture나 추정값으로 대체하지 않는다.
...
- API origin은 `KSKILL_SEOUL_WEATHER_RISK_API_BASE_URL`로만 설정한다. HTTPS origin을 사용하며 값은 명령행 인수, 문서, 로그에 넣지 않는다.
- API Key는 `KSKILL_SEOUL_WEATHER_RISK_API_KEY`로만 설정한다. helper는 `Authorization: Bearer` 헤더로 전송하며 값을 출력하거나 저장하지 않는다.
...
- `missing_api_key`, `missing_api_base_url`, `invalid_api_base_url`: 환경 설정 오류
